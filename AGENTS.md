...
For this project, the concrete list lives in ARCHITECTURE.md (`## EXTERNAL_BOUNDARY (this project)`) — Google's JWKS endpoint and Identity Services JS SDK, Kakao's OAuth/profile/unlink APIs and JS SDK, Naver's profile API and JS SDK. Treat that list as the authoritative one; do not redefine it here.
...
3. Read FRAMEWORK.md
4. Read task definition (from TASKS.md)
5. If the task touches a domain not covered by verified knowledge in PRODUCT_SPEC.md or ARCHITECTURE.md, or touches a section already marked `[UNVERIFIED]`: stop. Do not implement against an UNVERIFIED section. Require explicit human confirmation before continuing. (Today's open UNVERIFIED mark: Kakao's verified-email field shape — see ARCHITECTURE.md.)
6. Implement
...
9. Run all tests: `vendor/bin/phpunit`. Format with `vendor/bin/pint` before considering the task done.
10. Stop
...
The Kakao/Naver `Http::fake()` shapes currently in `tests/` are written from documented API shapes, not from a recorded live response — the Kakao email-verification field in particular is flagged `[UNVERIFIED]` in ARCHITECTURE.md and should be re-checked against a live response before being treated as settled.
...
Concretely for this package: no shared base class between KakaoProvider/NaverProvider until a third code-exchange-style provider is actually added; no generic "OAuth manager" layered on top of SocialLoginManager; no additional consent fields beyond terms/privacy/marketing unless PRODUCT_SPEC.md is updated first.
